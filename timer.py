...
while True:
	...
	while hour >3 and hour <9:
		hour =  int(datetime.datetime.now().strftime("%H"))
		time =  int(datetime.datetime.now().strftime("%H%M"))
		with open("/home/pi/Projects/html/homeauto/timer.txt") as timer:
			check=timer.read(1)
		if check=='1' and time ==430:
			switchoff()
			checkreset()
		if check=='2' and time ==500:
			switchoff()
			checkreset()
		if check=='3' and time ==530:
			switchoff()
			checkreset()
		if check=='4' and time ==600:
			switchoff()
			checkreset()
		if check=='5' and time ==830:
			switchoff()
			checkreset()
		sleep(60)
		#print time
